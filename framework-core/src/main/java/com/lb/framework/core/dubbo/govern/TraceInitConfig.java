...
public class TraceInitConfig {
					...
					if (config == null) {
						synchronized (lock) {
							...
							if (config != null) {
								try {
									// Path initFile =
									// Paths.get(TraceInitConfig.class.getClassLoader().getResource("trace.properties").toURI());
									initConfig(trace/* ,initFile */);
									TraceConfigFileMonitor monitor = new TraceConfigFileMonitor();
									// 监控根目录
									Path path = Paths
											.get(TraceInitConfig.class.getClassLoader().getResource("").toURI());
	...
	/**
	 * 备注：文件存在jar包中或者类路径中，直接用class.getClassLoader().getResourceAsStream(fileName
	 * )可以获取输入流
	 * 
	 * @param trace
	 */
	private static void initConfig(Trace trace/* ,Path resolvePath */) {
		LOG.info(Log.op("init trace filter").msg("init trace filter begin").kv("envInfo", EnvInfo.getEnvInfo())
				.toString());
		...
		LOG.info(Log.op("init trace filter").msg("init trace filter end").kv("envInfo", EnvInfo.getEnvInfo())
				.toString());
	}
