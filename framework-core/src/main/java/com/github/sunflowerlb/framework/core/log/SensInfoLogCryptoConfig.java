 ...
 *
 */
public class SensInfoLogCryptoConfig {
	...
	public void setSensInfoCryptoEnable(String sensInfoCryptoEnable) {
		...
		isSensInfoCryptoEnable = StringUtils.isEmpty(this.sensInfoCryptoEnable) ? Boolean.TRUE
				: Boolean.valueOf(this.sensInfoCryptoEnable);
		Log.setIsSensInfoCryptoEnable(this.isSensInfoCryptoEnable());
	}
	...
	static class SensInfoLogCryptoConfigFileMonitor extends AbstractFileMonitor {
		...
		public void process(Path file, Path resolvePath, Kind<Path> eventKind) {
			logger.info(Log.op("senslog.properties_create_or_modify").kv("file_path", resolvePath.toString())
					  .kv("operation", eventKind.name()).toString());
			try (FileInputStream fis = new FileInputStream(resolvePath.toFile());){
				...
				logger.error(Log.op("load_senslog.properties_io_exception")
						  .msg(e.getMessage())
						  .kv("className", SensInfoLogCryptoConfigFileMonitor.class.getName())
