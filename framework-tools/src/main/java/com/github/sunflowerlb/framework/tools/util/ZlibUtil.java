...
public class ZlibUtil {
    ...
    public static int uncompress(byte[] compressBytes, int offset, int len, OutputStream os)
            throws IOException {
        if (compressBytes == null || compressBytes.length < (offset + len)) return 0;
        InflaterInputStream iis =
                new InflaterInputStream(new ByteArrayInputStream(compressBytes, offset, len));
    ...
    public static byte[] compress(String inputStr) throws IOException {
        if (Strings.isNullOrEmpty(inputStr)) {
            return null;
        }

        byte[] unCompressbytes = inputStr.getBytes("UTF-8");

        byte[] compressBytes = compress(unCompressbytes, 0, unCompressbytes.length);
        log.info("UnCompress Size: " + unCompressbytes.length + "  Compress Size:"
